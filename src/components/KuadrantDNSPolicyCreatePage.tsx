...
import { getKindGroupLatestVersion } from '../utils/getModelFromResource'; // Import the helper
...
const KuadrantDNSPolicyCreatePage: React.FC = () => {
  ...
  const [apiVersion, setApiVersion] = React.useState<string>(''); // State for dynamic API version
  ...
  // Fetch dynamic API version using getKindGroupLatestVersion
  React.useEffect(() => {
    const fetchApiVersion = () => {
      getKindGroupLatestVersion('DNSPolicy', 'kuadrant.io')
        .then((result) => {
          setApiVersion(result.version || 'v1alpha1'); // Fallback to 'v1alpha1' if no version is found
        })
        .catch((error) => {
          console.error('Error fetching the latest version:', error);
          setApiVersion('v1alpha1'); // Fallback in case of an error
        });
    };
  ...
  const createDNSPolicy = () => {
    ...
    return {
      apiVersion: `kuadrant.io/${apiVersion}`, // Use dynamic API version
      kind: 'DNSPolicy',
      metadata: {
        name: policyName,
  ...
  if (nameEdit) {
    dnsResource = {
      groupVersionKind: {
        group: 'kuadrant.io',
        version: apiVersion, // Use dynamic API version
        kind: 'DNSPolicy',
      },
      isList: false,
      name: nameEdit,
