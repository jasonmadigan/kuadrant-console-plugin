...
const fetchConfig = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return defaultConfig; // Fallback on 404 or other non-ok statuses
    }

    const script = await response.text();

    // Inject the config.js script into the global window object
    const configScript = document.createElement('script');
    configScript.innerHTML = script;
    document.head.appendChild(configScript);

    return (window as any).kuadrant_config || defaultConfig; // Fallback if window.kuadrant_config is undefined
  } catch (error) {
    console.error('Error loading config.js:', error);
    return defaultConfig; // Fallback in case of fetch or script errors
  }
};
